...
#include <tuw_serial_arduino/TimeoutSerial.h>
#include <string>
#include <algorithm>
#include <iostream>
...
std::size_t TimeoutSerial::bytesWaiting(){
  return readData.size();
}

void TimeoutSerial::clear_tx(){
...
void TimeoutSerial::startTransmission(void) {
    char data = 0x00;
    asio::write(port,asio::buffer(&data,1));
}

void TimeoutSerial::endTransmission(void) {
    char data = 0x01;
    asio::write(port,asio::buffer(&data,1));
}

void TimeoutSerial::writeEscaped(const char *data, size_t size) {
    char esc[2] = {0x02, 0x00};
    for (int i = 0; i < size; i++) {
        if (data[i] == 0x00) {
            esc[1] = 0x30;
            asio::write(port,asio::buffer(esc,1));
            asio::write(port,asio::buffer(&esc[1],1));
        } else if (data[i] == 0x01) {
            esc[1] = 0x31;
            asio::write(port,asio::buffer(esc,1));
            asio::write(port,asio::buffer(&esc[1],1));
            ...
            asio::write(port,asio::buffer(&data[i],1));
        }
    }
}

int TimeoutSerial::readEscaped(char *data, size_t size) {
    char temp;
    for (int i = 0; i < size; i++) {
        read(&temp, 1);
        if (temp == 0x00) {
            ...
            read(&temp, 1);
            if (temp == 0x30) data[i] = 0x00;
            if (temp == 0x31) data[i] = 0x01;
            if (temp == 0x32) data[i] = 0x02;
            if (temp == 0x34) data[i] = 0x04;
            // assert(false);
