...
struct TMotor {
    ...
    std::string getToString() const {
        char buf[0xFF];
        sprintf ( buf, "[[ %6.4f, %6.4f], [ %6.4f, %6.4f]]", rps.actual, rps.target, pwm.actual, pwm.target );
        return std::string ( buf );
    }
    bool setFromString ( const std::string &str ) {
        int start = str.find ( "[" );
        ...
        std::string data = str.substr ( start+1, end-1 );
        boost::erase_all ( data, " " );
        if ( sscanf ( data.c_str(), "[%f,%f],[%f,%f]", &rps.actual, &rps.target, &pwm.actual, &pwm.target ) == EOF ) return false;
        return true;
    }
#endif
};
